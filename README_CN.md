...
Claude Code 将所有 bash 命令和对话内容存储在 `~/.claude/projects/` 目录下的会话文件中。**deja** 将这些内容索引到 SQLite 数据库，让你可以找到上周的命令、回顾之前的讨论决策，避免重复犯错。
...
**手动下载**：从 [releases](https://github.com/Michaelliv/cc-dejavu/releases) 获取二进制文件
...
# 只搜索思考内容
deja chat search "架构" --type thinking
...
所有数据本地存储在 SQLite 数据库 `~/.cc-dejavu/history.db`。
...
| 找特定 git 操作 | `deja search "git push" --cwd ~/projects/myapp` |

## 给 AI Agent 使用

运行 `deja onboard` 将使用说明添加到 `~/.claude/CLAUDE.md`：

```bash
deja onboard
```

这会添加 `deja search`、`deja list` 和 `deja chat search` 的使用指南。
...
在 Claude Code 的 SessionEnd 钩子中使用 `deja ingest --candidates --format json`：
...
**deja** - 源自 *déjà vu*，意为"似曾相识"。让你看到之前执行过的命令和对话。
